...
export const packet: LearningPacket = {
  nodeId: "l2-stats-mle",
  whyNow:
    "'Training a model' formally means maximum likelihood: cross-entropy IS negative log-likelihood, least squares IS MLE under Gaussian noise. See that once, on paper, from a blank page, and loss functions stop being arbitrary for the rest of your career. This node deliberately stops there — no NHST, no confidence-interval machinery, no bootstrap: MLE and Bayesian updating are the only statistics this path consumes directly.",
  diagnostic: {
    prompt:
      "Cold: (a) why maximize LOG likelihood — give two real reasons; (b) derive the MLE of λ for n exponential samples; (c) is the likelihood a probability distribution over θ? Clean on all three → skip straight to Pset 6.",
    minutes: 10,
  },
  orient: {
    ...
    unverified: true,
    whySelected:
      "The canonical gentle on-ramp: slide a Gaussian over data and watch the likelihood change. Deliberately derivation-free — the deriving is your job, minutes from now.",
  ...
  coreWatch: [
    {
      ...
      unverified: true,
      whySelected:
        "Locate it by title in the linked video index (no direct URL verifiable this session). Watch with the orient video: it pre-empts THE canonical confusion — likelihood is not a pdf over θ — before the formalism can install it.",
  ...
  coreRead: [
    ...
    },
    {
      ...
      minutes: 40,
      whySelected: "THE core read — ~10 pages at exactly engineer depth; no other free source scaffolds 'loss functions are MLE in disguise' this directly.",
    },
    {
  ...
  practice: [
    {
      prompt:
        "In gaussian-explorer: generate 20 noisy points, fit μ and σ by EYE with the sliders and write your guesses down; then compute μ̂ and σ̂ by MLE and compare. The eye is good at μ and systematically bad at σ — feel the likelihood out-fit you.",
      minutes: 15,
    },
    {
    ...
    {
      prompt:
        "Pset 6, Problems 1–2 — the continuous (gamma-form) likelihood, and 'use MLE to develop Gauss' method of least squares'. This is the main event: full effort before any peeking at solutions.",
      source: "https://ocw.mit.edu/courses/18-05-introduction-to-probability-and-statistics-spring-2022/mit18_05_s22_pset06.pdf",
      minutes: 45,
    },
    {
      prompt: "Plot the likelihood surface for 10 coin flips, then 1000 — watch it sharpen, and connect the width to standard error.",
      minutes: 20,
    },
    {
  ...
  derive: {
    spec: "On paper, from the log-likelihood, no template open: derive p̂ = k/n for n Bernoulli flips, then μ̂ and σ̂² for n Gaussian samples — every differentiation step written, plus an argument (second derivative or shape) that you found a maximum.",
    checks: [
  ...
  implement: {
    spec: "mle_check.py: (1) numerically maximize the same two log-likelihoods on simulated data — grid search, or the gradient ascent from l2-optimization if you've built it — and confirm agreement with your closed forms; (2) one Bayesian update table in Reading 11 style for a coin with prior {fair: 0.4, biased: 0.6}, updated over a short flip sequence.",
    checks: [
  ...
  stuck: {
    ...
    note: "Stuck on the exponential-λ diagnostic instead: the matching walkthrough is 'Maximum Likelihood for the Exponential Distribution, Clearly Explained! V2.0' — https://www.youtube.com/watch?v=p3T-_LMrvBc (duration unverified). These exist to un-stick you, never to replace your own derivation.",
  },
  deepen: [
    {
  ...
  prove: {
    task: "The node's mastery gate, closed book, from a blank page: (1) assume yᵢ = a·xᵢ + b + εᵢ with ε ~ N(0, σ²); show that maximizing the likelihood is equivalent to minimizing Σ(yᵢ − a·xᵢ − b)² — then, and only then, check yourself against the posted Pset 6 solution; (2) show that for binary labels under a Bernoulli model, negative log-likelihood is exactly cross-entropy. You will reuse both arguments in every ML conversation for years.",
    criteria: [
      "The Gaussian pdf is written into the likelihood and the log taken before any calculus",
      "The σ and constant terms are explicitly identified as argmax-irrelevant — that step IS the theorem",
