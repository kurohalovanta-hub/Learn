...
export const packet: LearningPacket = {
  nodeId: "l6-kalman",
  whyNow:
    "The Kalman filter is the most deployed algorithm in robotics history — and by now you own every part of it: it is literally your Bayes filter with Gaussian beliefs, and your state-space model as the motion model. This node builds it from scratch, 1D to multivariate, and then teaches the test almost nobody teaches: NEES/NIS consistency — the difference between a filter that works and one that merely draws smooth plots while lying about its uncertainty.",
  diagnostic: {
    prompt:
      "Cold, before any reading: open the kalman-1d widget and run labs 1–5 (predict-only spread; single-update snap; R↑ makes updates timid; Q↑ makes the filter lean on measurements; σ's steady rhythm). Then on paper: what do K→0 and K→1 each say about sensor trust? Write the innovation and its covariance in symbols.",
    minutes: 12,
  },
  coreRead: [
    {
      ...
      minutes: 270,
      whySelected:
        "The community's default answer for learning KF by building (19.2k stars): executable, exercise-bearing with answers, 1D → multivariate in exactly this node's order — and Ch 8's NEES/NIS methodology is precisely the honest-filter objective no video course teaches.",
    },
    {
      ...
      minutes: 15,
      whySelected: "Deliberately demoted to conditional — do not spend an hour re-reading what you already gated on.",
    },
  ],
  recall: [
    {
  ...
  practice: [
    {
      prompt:
        "Mis-tune R by 100× in both directions on your 1D filter: show the overconfident filter (R too small — jumpy state, NIS blowing through the chi-square bounds) and the sluggish one (R too large — smooth, late, NIS collapsed below the band). Keep both NIS plots; this is what a lying filter looks like from now on.",
      minutes: 30,
    },
    {
      prompt:
        "Labbe's in-notebook exercises for Ch 4–8, commit-before-reveal: write your answer down before opening his. Every exercise you skip is a hole the mastery test finds.",
      source: "https://github.com/rlabbe/Kalman-and-Bayesian-Filters-in-Python",
      minutes: 45,
    },
  ],
  derive: {
    spec: "On paper: multiply two 1D Gaussians N(μ₁, σ₁²)·N(μ₂, σ₂²), complete the square, and show the posterior mean is the precision-weighted average μ = (σ₂²μ₁ + σ₁²μ₂)/(σ₁² + σ₂²) with variance below both inputs. Then map the result symbol-by-symbol onto the KF update: identify the gain K = P̄/(P̄ + R), the innovation, and show x = x̄ + K(z − x̄) is your formula in disguise.",
    checks: [
  ...
  implement: {
    spec: "est.py: 2D constant-velocity tracker from noisy position measurements — predict/update written from scratch in matrix form (F, H, Q, R, P explicit); FilterPy run alongside as TEST ORACLE ONLY, asserting your x and P match its every step; NEES and NIS computed over Monte-Carlo runs and plotted against chi-square bounds.",
    checks: [
  ...
  stuck: {
    ...
    note: "Alternate voice: Cyrill Stachniss's Kalman/EKF lecture (via www.ipb.uni-bonn.de/teaching/ — not re-verified this session). If local Jupyter fights you, the repo README links nbviewer and binder for zero-install running.",
  },
  deepen: [
    {
      ...
      minutes: 45,
      whySelected: "A genuinely different axis than this node's Bayes-first build — deliberately parked here, not core, so it lands when the duality question exists.",
    },
    {
  ...
  prove: {
    task: "Gold gate, two parts, both cold: (1) BLANK FILE — rebuild the 2D constant-velocity tracker without opening est.py or Labbe, run 100 Monte-Carlo runs, NEES within the chi-square bounds. (2) On paper: the 1D Kalman update derived as a product of two Gaussians, square completed, gain identified.",
    criteria: [
      "Written from a blank file — transcription is the exact failure mode this rule exists to catch",
  ...
  transfer: {
    task: "Two-sensor fusion: add a second position sensor to the same tracker with a 10× different R and a different update rate. Update on each measurement's arrival with its own H and R. Show (a) the fused posterior beats either sensor alone on RMSE, and (b) NIS stays inside its bounds for BOTH sensors — precision-weighting generalized beyond the textbook setting.",
    criteria: [
