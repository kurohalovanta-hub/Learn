...
export const packet: LearningPacket = {
  ...
  whyNow:
    "Losses are expectations, policies are distributions, sensor noise is Gaussian, and eval variance across rollouts is the CLT wearing a lab coat. This is the vocabulary layer every ML and robotics paper is written in — and its payoff here is quantitative: after this node, 'my success rate moved 8 points between evals' becomes a computation (σ/√n), not a feeling.",
  diagnostic: {
    prompt:
      "Cold: (a) Var(3X−2) in terms of Var(X); (b) E[X+Y] for DEPENDENT X, Y — legal or not?; (c) sketch N(2, 0.25) and mark the ±1σ mass; (d) the pdf of X at 0.3 is 2.4 — is that a probability? All four clean → jump straight to the PRACTICE psets.",
    minutes: 10,
  },
  orient: {
    ...
    unverified: true,
    whySelected:
      "Builds the PMF habit on a distribution you can count — and quietly pre-loads the likelihood curves that l2-stats-mle will turn into MLE.",
  ...
  coreWatch: [
    {
      ...
      minutes: 31,
      whySelected:
        "The node's conceptual centerpiece. Watch AFTER Readings 4a–5c and BEFORE Reading 6b — the video supplies the picture (Galton-board convolutions up to the σ/√n scaling), 6b then reads as confirmation. Long, but this is the exact concept the mastery test hinges on.",
      leaveWith: [
  ...
  coreRead: [
    ...
    },
    {
      ...
      minutes: 70,
      whySelected:
        "The node's spine: each reading maps one-to-one onto an objective (4a→PMF, 4b/5a→E/Var, 5b/5c→pdf + zoo), with per-reading self-checks built in.",
    },
    {
  ...
  practice: [
    {
      prompt:
        "In the gaussian-explorer instrument: drag μ and σ, watch the pdf/CDF respond; verify the ±1σ ≈ 68% mass claim your l2-integrals node found numerically; then simulate your own sensor noise in NumPy and match the widget's Gaussian to its histogram.",
      minutes: 15,
    },
    {
      prompt: "18.05 Pset 2 in full — variance of discrete RVs, continuous RVs. On paper before checking anything.",
    ...
    {
      prompt:
        "Pset 5 — the covariance/correlation problems plus ONE worked joint-CDF problem; skip the remaining joint-CDF technicalities (the robotics path needs covariance fluency, not joint-CDF manipulation).",
  ...
  stuck: {
    alternate: {
      ...
      minutes: 12,
      unverified: true,
      whySelected: "Exactly the pdf-vs-probability block, with densities introduced honestly — for when continuous RVs are where you stalled.",
    },
    alternateRead: {
  ...
  prove: {
    task: "Unassisted, closed book: (1) derive E and Var of a Bernoulli(p) and a Uniform(a,b) from the definitions — sum and integral written out; (2) simulate a 50-rollout policy evaluation twice with true success p=0.7, report the two differing success rates, and explain the discrepancy quantitatively with the CLT — σ/√n with actual numbers — stating how many rollouts it takes to halve the error bar. This is the eval-statistics insight the whole field just relearned.",
    criteria: [
  ...
  transfer: {
    task: "An IMU reads z = true + ε with ε ~ N(0, σ²). (1) What is the distribution of the mean of k readings, and how many readings do you need for a 95% interval of ±0.1σ? (2) The inverse question: your robot's grasp success differs by 8 points between two 20-trial evals — signal or noise? Decide with a computed standard error, not a feeling.",
    criteria: [
