...
export const packet: LearningPacket = {
  ...
  whyNow:
    "Loops are your first real algorithms, and your first off-by-one bugs. This node teaches you to run code in your head, line by line, instead of running it and hoping. You will learn two patterns, accumulator and search-with-early-exit, that carry you from FizzBuzz to training loops.",
  diagnostic: {
    prompt:
      "Hand-trace this cold: for i in range(3): for j in range(i): print(i, j). Write the EXACT output line by line, then run to check. Second question: what does while True: with a break inside mean, and when does that loop end?",
    minutes: 5,
  },
  coreRead: [
    {
    ...
    },
    {
      ...
      minutes: 25,
      whySelected: "Read this, then go straight to Exercism 'Making the Grade'. It names the accumulator and search patterns.",
    },
  ],
  recall: [
  ...
  practice: [
    {
      prompt:
        "One Exercism exercise for each read. After ch 5, do 'Ghost Gobble Arcade Game' (bools) and 'Meltdown Mitigation' (conditionals: a nuclear-reactor control system, your first controller). After ch 7, do 'Making the Grade' (loops).",
      source: "https://exercism.org/tracks/python/concepts",
      minutes: 45,
    },
    {
      prompt:
        "From a blank file each time, no skeletons, nothing looked up: FizzBuzz plus two variants, Collatz sequence length, prime tester, number-guessing game. Before writing each one, put its pattern in a one-line comment at the top: 'accumulator' or 'search with early exit'. Predict every trace before running, and journal every wrong trace.",
      minutes: 95,
    },
    {
      prompt:
  ...
  implement: {
    spec: "patterns.md. After the practice set, write one line per program naming its pattern (accumulator / search / both), its state variable(s), and their update. This is the vocabulary you will use to READ unfamiliar loops from here on.",
    checks: [
  ...
  stuck: {
    alternate: {
      ...
      minutes: 30,
      whySelected: "The verified backup path. Notes first (15–25 min, structured), then the lecture segment at 1.5–2×, only the failing topic.",
      unverified: true,
    },
    note: "If a trace keeps surprising you, slow down and run it on paper with a variables table. Do not re-read yet.",
  },
  deepen: [
    {
  ...
  prove: {
    task: "Node mastery test, one sitting: print a text histogram of dice-roll sums over 10,000 simulated two-die rolls, using loops and the random module only, no Counter, no numpy. It must work on the first or second run. Paste the printed histogram as your evidence.",
    criteria: [
      "Working on the first or second run, with any fix driven by the traceback or a hand-trace, not trial and error",
