...
export const packet: LearningPacket = {
  ...
  whyNow:
    "Datasets are lists of dicts. Configs are dicts. Trajectories are lists of tuples, so picking the right structure is half the work. The big idea here is aliasing: two names pointing at one object, the classic 'why is my list changing?!' wall. You will read it, reproduce it, and prove it with id(). Comprehensions become your go-to, and you write each one as a loop first so it never feels like magic.",
  diagnostic: {
    prompt:
      "Predict, then run: a = [1, 2]; b = a; b.append(3); print(a). Now try the b = a[:] version. Explain the memory model in two sentences. Also, what does xs = xs.sort() leave in xs? Last thing, before you read anything, draw the names-to-objects diagram for these snippets and keep it. The reading will confirm or correct it.",
    minutes: 20,
  },
  coreRead: [
    {
  ...
  practice: [
    ...
    {
      prompt:
        "Reproduce the aliasing bug (two names, one list), then fix it three ways: a[:], list(a), copy.copy(a). Prove with id() that each fix makes a separate object.",
      minutes: 15,
    },
    {
      prompt:
    ...
    {
      prompt: "Rewrite three of your l1-control-flow loops as comprehensions, but only where the comprehension reads better. Anything nested past two levels stays a loop.",
      minutes: 10,
    },
  ],
  implement: {
    spec: "The structure-choice card: for five mini-scenarios (an experiment config, a robot trajectory, a set of visited states, a leaderboard, a graph adjacency), pick list, tuple, dict, or set and defend each choice in one line.",
    checks: [
  ...
  stuck: {
    ...
    note: "Print id() before and after any operation that confuses you, and the model becomes visible. Step through similar snippets in Runestone FoPP's CodeLens or Python Tutor (find either by name). If it still won't click, watch Ned Batchelder's talk 'Facts and Myths about Python Names and Values', found by its exact title.",
  },
  deepen: [
    {
  ...
  prove: {
    ...
    criteria: [
      ...
      "Mutating a filtered result leaves the source records intact; show it (no aliasing accident)",
