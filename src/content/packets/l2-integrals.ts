...
export const packet: LearningPacket = {
  nodeId: "l2-integrals",
  whyNow:
    "Just enough integration to read ∫p(x)dx = 1 and 𝔼[X] = ∫x p(x)dx, the notation half of probability. Hold the picture of adding up tiny changes, not 'integral = antiderivative' memorized; that is what carries over to expectations and to turning velocity into position. This node stays small on purpose and gates at silver. Grinding integration techniques is the classic Calc II time sink, so skip it.",
  diagnostic: {
    prompt:
      "∫₀¹ 3x² dx in your head; what is d/dx ∫₀ˣ f(t)dt?; one u-sub: ∫2x·cos(x²)dx. Get all three clean and jump straight to IMPLEMENT and PROVE IT.",
    minutes: 4,
  },
  coreWatch: [
    {
      ...
      minutes: 20,
      whySelected: "Builds the FTC from the moving-car problem, the one picture that carries over to expectations. Pause at the FTC reveal and try to state it yourself first.",
  ...
  coreRead: [
    {
      ...
      minutes: 25,
      whySelected: "The exact subset that covers the whole objective list, with none of the Calc II technique machinery.",
    },
    {
  ...
  practice: [
    { prompt: "Paul's Substitution Rule practice set (indefinite): work the odd-numbered problems on paper and check each against the full solutions. Stop when it feels easy, not when you finish the set.", source: "https://tutorial.math.lamar.edu/problems/calci/substitutionruleindefinite.aspx", minutes: 30 },
    { prompt: "Paul's Substitution Rule practice set (definite): do a handful. The point is transforming the limits along with the variable.", source: "https://tutorial.math.lamar.edu/Problems/CalcI/SubstitutionRuleDefinite.aspx", minutes: 20 },
  ],
  implement: {
    spec: "Build a trapezoid integrator from scratch (plain loop first, then vectorized). Check it against the exact ∫₀¹ 3x² dx = 1. Then integrate the standard Gaussian pdf on [−1,1] and watch ≈0.683 fall out; that is your first look at '±1σ holds 68%', which l2-random-variables needs one node from now. Once it works, compare your loop to numpy.trapezoid.",
    checks: [
  ...
  stuck: {
    alternate: {
      ...
      minutes: 10,
      whySelected: "The average-of-a-function view, which is the mental model behind expectations. Find Ch. 9 in the series playlist.",
      unverified: true,
    },
    alternateRead: {
  ...
  prove: {
    task: "Closed book: (1) evaluate ∫x e^{x²}dx by substitution; (2) write a short paragraph on why 𝔼[X] = ∫x p(x)dx is 'weighted average, continuum edition'. Connect the sum-of-slices picture to the discrete Σx·p(x).",
    criteria: [
      "u = x² spotted from the inner-function/derivative pair, the ½ constant handled, +C present",
      "The explanation says what p(x)dx is (the probability in a slice) and why the x-weighted sum turns into an integral",
      "Nothing outside the node's scope needed; if you reached for a Calc II technique, you drifted",
    ],
    minutes: 20,
  },
  transfer: {
    task: "Odometry: a wheeled robot drives with v(t) = sin t over [0, π]. Get the position change two ways, by exact integral and by your trapezoid code. Halve the step size, watch the error drop to a quarter, and say why.",
