...
export const packet: LearningPacket = {
  nodeId: "l1-classes",
  whyNow:
    "A class is just state plus the functions allowed to change that state. Every PyTorch nn.Module and every gym Env you will read has that shape. Here you get comfortable reading and writing it: what self really is, what __init__ does, and the super().__init__() contract. A function is still your default; a class has to earn its place.",
  diagnostic: {
    prompt:
      "Cold, no notes: (1) What is self? (2) In a subclass of an nn.Module-style base, what does super().__init__() do, and why must it run before you use inherited state? (3) A class body contains items = []; two instances each call inst.items.append(...). Predict what both instances now see, and why.",
    minutes: 10,
  },
  coreRead: [
    {
    ...
    },
    {
      ...
      minutes: 10,
      whySelected: "First-party and exact on the two spots where people usually get stuck here.",
    },
  ],
  recall: [
  ...
  practice: [
    {
      prompt:
        "ORIENT, before any reading: skim a gym-style Env skeleton (reset(), step(action), internal state) and write ONE sentence on what the class is FOR, which is state plus the changes allowed to it. This answers 'when would I even use a class?' before the syntax starts.",
      minutes: 10,
    },
    {
    ...
    {
      prompt:
        "The shared-class-attribute experiment: a mutable class attribute (items = [] in the class body) versus an instance attribute created in __init__. Append through one instance, print through another, and prove what happened with __dict__ and id().",
      minutes: 10,
    },
    {
      prompt:
        "Subclass a tiny Base whose __init__ sets up state the child needs. Show the child break without super().__init__(), then work with it. This is the nn.Module contract in miniature.",
      minutes: 10,
    },
    {
      prompt: "Exercism 'Ellen's Alien Game', the track's classes exercise. It drills class versus instance attributes head-on.",
  ...
  stuck: {
    alternate: {
      ...
      minutes: 30,
      whySelected: "Builds up from a plain dict to a class, motivation first. Use this video path if reading alone doesn't click. Topic markers at 1.5–2×, failing topic only.",
      unverified: true,
    },
    note: "Want a second voice? Corey Schafer's 'Python OOP Tutorial' parts 1–2, found by title (URLs unverified this session). Part 2 covers the shared-class-attribute trap head-on.",
  },
  deepen: [
    {
  ...
  prove: {
    task: "Node mastery test: design a Particle class for a toy simulator. It holds state (position, velocity, mass), a step(dt) that advances that state in place, and energy() computed from the state. Add ONE subclass that changes behavior (say, drag in step) and calls super().__init__(). Justify every method's placement in one line each. Run 10 steps and paste the printed position/velocity/energy trace.",
    criteria: [
