...
export const packet: LearningPacket = {
  nodeId: "l1-numpy",
  whyNow:
    "Every tensor you will ever feed a network obeys the rules you learn here — the ndarray IS the PyTorch tensor model with a different import. Shapes, broadcasting, axis semantics, views vs copies: learn to predict them cold now, and batched deep-learning code stops being copied incantation and becomes something you write. Highest-leverage 8 hours in L1.",
  diagnostic: {
    prompt:
  ...
  orient: {
    ...
    minutes: 8,
    whySelected:
      "The memory-layout framing of WHY numpy is fast, at ideal orientation length. Prefer text? Skim Jay Alammar's picture-first Visual Intro instead (jalammar.github.io/visual-numpy, ~10 min).",
    leaveWith: [
  ...
  coreRead: [
    ...
    },
    {
      ...
      minutes: 10,
      whySelected: "THE authority on the two rules. Its closing example is literally this node's k-nearest mastery pattern — you will have to reproduce the move cold.",
    },
    {
  ...
  practice: [
    {
      prompt:
        "The node's 15 predict-then-run broadcasting expressions: for each, write the predicted result shape (or 'error') BEFORE running — mix (n,), (n,1), (1,n) and 3-D operands — then verify in the REPL and autopsy every miss.",
      minutes: 25,
    },
    {
      prompt:
        "numpy-100 exercises #25 (in-place boolean negate), #37 (5×5 row values — broadcasting), #44 (cartesian→polar), #52 (point-by-point distances), #58 (subtract row means — axis + keepdims), #64 (add via index vector), #71 ((5,5,3)×(5,5) — the newaxis test). Predict shapes before running; hints/solutions variants live in the repo.",
  ...
  prove: {
    task: "Node mastery test, closed references: vectorize three loop-based routines — moving average, standardize-by-column, k-nearest by distance matrix — with zero Python loops and outputs matching the loop versions. The broadcasting doc's vector-quantization example is the same pattern: do this WITHOUT reopening it.",
    criteria: [
  ...
  transfer: {
    task: "Given an (H,W,3) image array, standardize each channel to zero mean and unit std using only broadcasting — per-channel stats via axis=(0,1) with keepdims. Unfamiliar data, same rules. Predict every intermediate shape before running.",
    criteria: [
