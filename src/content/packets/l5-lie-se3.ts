...
export const packet: LearningPacket = {
  nodeId: "l5-lie-se3",
  whyNow:
    "This is where the math gets harder. Homogeneous transforms hold rotation and translation as one object, and exp/log maps let you differentiate, interpolate and optimize poses without ever leaving the manifold. Almost every modern estimation and robot-learning paper assumes this quietly. Here the pieces line up: the author teaching his own paper, the paper's boxed identities as your test suite, and his own library (manif) to check your answers against. Give it the full block. These symbols are easy to recognize and easy to fake, so slow down and prove them to yourself.",
  diagnostic: {
    prompt:
      "Cold, no notes: (1) What is the log of a rotation matrix, geometrically? What kind of object comes out, and what do its 3 numbers mean? (2) Why can't you optimize a rotation by gradient-stepping its 9 matrix entries, and why doesn't projecting back fix it? (3) Is T ⊞ ξ the same thing as ξ ⊞ T? If the third question looks meaningless, that is exactly what this node fixes.",
    minutes: 10,
  },
  coreWatch: [
    {
      ...
      minutes: 60,
      whySelected:
        "The author teaching his own paper. He makes concrete every symbol you are about to implement, in the exact notation of the paper and of manif. Watch it in full and pause to take notes. No 2× skimming; the curation record protects this hour.",
      leaveWith: [
        ...
        "the thesis: the part of Lie theory you actually need is small and computational",
      ],
      unverified: true,
    },
  ],
  coreRead: [
    {
      ...
      minutes: 10,
      whySelected: "The one picture everything else in this node builds on. Get it first, and the lecture lands on prepared ground.",
    },
    {
      ...
      minutes: 195,
      whySelected:
        "Its ⊞/⊟ and Jacobian conventions are the conventions of manif and of modern estimation papers. Learn them once and use them for the rest of the curriculum. The appendix tables become a desk reference you keep coming back to.",
    },
  ],
  recall: [
    {
  ...
  practice: [
    {
      prompt:
        "During the working read, verify every boxed identity numerically on random inputs as you meet it: exp/log closed forms, adjoint, the ⊞/⊟ definitions. (This time counts inside the core-read block. Reading and testing are one activity here.)",
    },
    {
      prompt:
        "Geodesic vs naive: interpolate two rotations two ways, by R(t) = R₁·exp(t·log(R₁ᵀR₂)) and by entrywise lerp of the matrices. Animate both triads and check at every t which path stays in SO(3) (RᵀR = I, det = +1). Watch the lerp path shear and scale.",
      minutes: 30,
    },
    {
      prompt:
  ...
  implement: {
    spec: "Complete se3.py: SO(3)+SE(3) exp and log in closed form, hat/vee, adjoint, and ⊞/⊟ in BOTH conventions. Implement right/local (T ⊞ ξ = T·exp(ξ^)) and left/global (exp(ξ^)·T), each labeled clearly and never mixed. Property suite: exp(log(T)) = T; log(exp(ξ)) = ξ near identity; T·exp(ξ^)·T⁻¹ = exp((Ad_T ξ)^); (T ⊞ ξ) ⊟ T = ξ in each convention. Check every operation against manif's Python bindings as your oracle (github.com/artivis/manif).",
    checks: [
  ...
  derive: {
    spec: "On paper, references closed: (1) From the series exp([ω]×θ) = I + [ω]×θ + [ω]×²θ²/2! + …, use the identity [ω]×³ = −[ω]× to collapse the even and odd terms into Rodrigues' formula R = I + sinθ[ω]× + (1−cosθ)[ω]×². (2) Invert it to derive log: get θ from tr(R) = 1 + 2cosθ and the axis from the skew part R − Rᵀ. State exactly where the extraction breaks down (θ → 0 and θ → π) and what your code must do at each (Taylor branch near 0, separate axis extraction near π). (3) In one tight paragraph, explain why the v in a twist (v, ω) is not the velocity of the body origin. Write v = ṗ − ω×p and say which point v actually describes.",
    checks: [
  ...
  stuck: {
    alternate: {
      ...
      minutes: 60,
      whySelected: "The same material, told a second time to a different audience with different Q&A. When the first pass didn't land, hear it again before pushing symbols.",
      unverified: true,
    },
    alternateRead: {
    ...
    note: "Third phrasing if needed: Bang-Shien Chen's condensed companion notes (dgbshien.com/docs/blogs/lie-theory.pdf), evidence this paper gets studied, not skimmed. If YouTube is unreachable, the LAAS PeerTube mirror carries the lecture (peertube.laas.fr/videos/watch/52ffc81c-09ce-41e9-8d6c-8c8d2b664d71).",
  },
  deepen: [
    {
    ...
    },
    {
      ...
      minutes: 30,
      whySelected: "The same ⊞/⊟ machinery, one group bigger. A preview of why this approach scales where ad-hoc pose parameterizations collapse.",
    },
  ],
  prove: {
    task: "Run your 30-case property suite on the finished module: SO(3)+SE(3) exp/log round trips (near identity AND far), adjoint identities, and ⊞/⊟ inverse relations in both conventions. Everything green, with manif agreeing on every shared operation. Then, closed book, write one paragraph: why does optimization on rotations use local coordinates?",
    criteria: [
      "All 30 cases green at the declared tolerances (1e-9 near identity)",
      "Right and left conventions are covered by tests that would FAIL if the two were swapped, so the suite can tell them apart",
  ...
  transfer: {
    task: "Open diffik.py in kevinzakka/mjctrl (github.com/kevinzakka/mjctrl) and find the line(s) computing the orientation error. State which ⊟ convention the code uses, local/body or global/space frame, and justify it from which side the difference is composed and where the log is taken. Then state precisely what would change in the controller's behavior if you flipped the convention and touched nothing else.",
    criteria: [
